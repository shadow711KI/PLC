...
function ToggleSwitch({
  ...
  return (
    <div className={`toggle-switch ${disabled ? 'toggle-switch--disabled' : ''}`}>
      {label && (
        <span className={`toggle-switch__label toggle-switch__label--${size} ${checked ? 'toggle-switch__label--checked' : ''}`}>
          {label}
        </span>
      )}
      <div
...
with open('src/components/common/ToggleSwitch.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Optimized: ToggleSwitch.tsx")
