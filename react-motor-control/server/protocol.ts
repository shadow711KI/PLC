...
export function parseSPSStatusResponse(
  ...
  for (let motorNr = 1; motorNr <= motorCount; motorNr++) {
    ...
    if (posOffset + 1 < dataFrame.length) {
      ...
      let automatik: boolean | undefined = undefined;
      ...
      const logMsg = `${timestamp} [SPS-STATUS] ${spsName || ''} Motor: ${displayName} | Status-Bytes: ${b1.toString(16).padStart(2, '0').toUpperCase()} ${b2.toString(16).padStart(2, '0').toUpperCase()} | Status: ${status} | Automatik: ${automatik === undefined ? '-' : (automatik ? 'AN' : 'AUS')}`;
      console.log(logMsg);
      try {
